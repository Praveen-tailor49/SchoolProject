...
import { ADD_TOKEN, TOAST_DISMISS, TOAST_LOADING, TOAST_UPDATE } from '../../action/utils'
import { LOGIN_USER, SIGN_USER } from '../../action/api'

export default function AuthPage({ navigate, dispatch }) {
  const [isSignup, setIsSignup] = useState(false)
  const [loading, setLoading] = useState(false)
  const [form, setForm] = useState({ name: '', email: '', password: '', confirm: '', schoolName: '' })
  ...
  const handleSubmit = () => {
    ...
    const { name, email, password, schoolName } = form
    const toastId = TOAST_LOADING()
    let obj = {};

    if (isSignup) obj = { user_password: password, user_email: email, full_name: name, school_name: schoolName, created_at: new Date() }
    else obj = { user_password: password, user_email: email }

    HttpClient.post(isSignup ? SIGN_USER : LOGIN_USER, obj).then((res) => {
      const { data, status } = res;
      console.log(res);
      
      if (status == 200) {
        if (isSignup) setIsSignup(false)
        else { navigate('/dashboard'); dispatch(ADD_TOKEN(data.token)) }
        TOAST_UPDATE(toastId, "success", data.message);
      }
    }).catch(err => TOAST_UPDATE(toastId, "error", err))
      .finally(() => setTimeout(() => TOAST_DISMISS(toastId), 200))
  }
        ...
        <div className="w-full flex items-center justify-center">
          <div className="w-full max-w-md bg-white rounded-2xl shadow-lg border p-6">
            <div className="flex items-center justify-between">
              ...
              <div>
                <button onClick={() => setIsSignup((s) => !s)} className="text-sm px-3 py-1 rounded-lg bg-slate-100 hover:bg-slate-200 transition">
              ...
              {isSignup && (
                <>
                  <div>
                    ...
                    <input value={form.confirm} onChange={update('confirm')} className={`mt-1 block w-full rounded-md border p-2 focus:outline-none focus:ring-2 focus:ring-indigo-300 ${errors.confirm ? 'border-red-300' : 'border-slate-200'}`} type="password" placeholder="Repeat your password" aria-invalid={errors.confirm ? 'true' : 'false'} />
                    {errors.confirm && <p className="mt-1 text-xs text-red-600">{errors.confirm}</p>}
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-700">School Name</label>
                    <input value={form.schoolName} onChange={update('schoolName')} className={`mt-1 block w-full rounded-md border p-2 focus:outline-none focus:ring-2 focus:ring-indigo-300 ${errors.confirm ? 'border-red-300' : 'border-slate-200'}`} type="text" placeholder="Enter your school name " aria-invalid={errors.confirm ? 'true' : 'false'} />
